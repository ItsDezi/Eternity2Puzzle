	...
	import java.util.*;
	import java.io.File;
public class board_8x8 {

		private Piece[][] b = new Piece[10][10];//board will be surrounded by "artificial" edge pieces
		private Piece[] all_pieces = new Piece[64];
		int number_of_rows = 8;//number of real pieces(not artificial edge ones)
		int n = 8;//since its always an nXn grid, I just added this to simplify things
		int number_of_columns = 8;//number of real pieces(not artificial edge ones)
		int pieceID = 0;
		Stack<Piece> stack = new Stack<Piece>();//backbone of the backtracking
		int[][] stack_height = new int[8][8];
		//15 different pieces in the 10X10
		int row = 1;//W.R.T board b
		int col = 1;//W.R.T board b
		
		Piece[] arr0 = new Piece[32];//all pieces with a side of value 0
		...
		Piece[] arr4 = new Piece[24];//all pieces with a side of value 4
		Piece[] arr5 = new Piece[24];//all pieces with a side of value 5
		Piece[] arr6 = new Piece[24];//all pieces with a side of value 6
		Piece[] arr7 = new Piece[24];//all pieces with a side of value 7
		Piece[] arr8 = new Piece[24];//all pieces with a side of value 8
		Piece[] arr9 = new Piece[24];//all pieces with a side of value 9
		Piece[] arr10 = new Piece[24];//all pieces with a side of value 10
		Piece[] arr11 = new Piece[24];//all pieces with a side of value 11
		...
		public board_8x8(String fileName)
		{
			int count = 0;
			//String[] piece_reader = new String[all_pieces.length];
			Scanner ss;
	        ...
	        public void sort()
	        {
	        	...
	        	while(row < b.length-1) {
	        		...
	        		while(col<b.length-1)
	        		{
	        			...
	        			if(col == 11)
	        			{
	        				break;//leaves inner loop to increment row and set column back to 1
	        			}
	        			//if(col>=2)//PROBLEM(has to be greater than 2 so stack index doesnt pull out of bounds array)
	        			//{
		...
		public static void main(String[] args) {
			...
			board_8x8 bb = new board_8x8("sides_8x8.txt");
...
//time and memory measurement tactics source:https://www.vogella.com/tutorials/JavaPerformance/article.html
